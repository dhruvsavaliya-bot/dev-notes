# 📚 dev-notes

Auto-curated developer knowledge base — fresh content added **four times daily**
from GitHub Trending, Hacker News (100+ points), and dev.to's top posts.

**184 entries and counting** · Last updated: 2026-08-10

## Categories

| Section | Entries |
|---|---|
| [Trending Projects](trending-projects/projects.md) | 71 |
| [AI / LLM Notes](ai/notes.md) | 14 |
| [Reading List](articles/reading-list.md) | 47 |
| [Coding Tips](coding-tips/tips.md) | 45 |
| [Language Notes](languages/notes.md) | 7 |

## Latest additions

- **2026-08-10** · *System Design & Architecture* — [Restate's Single Binary vs Temporal's Cluster: When the](https://dev.to/andriiboyko/restates-single-binary-vs-temporals-cluster-when-the-lighter-engine-wins-3635)
- **2026-08-10** · *Hacker News Picks* — [Auto mode is now the default in Claude Code](https://news.ycombinator.com/item?id=49239021)
- **2026-08-10** · *Hacker News Picks* — [I made tinnitus my friend, then it disappeared [video]](https://news.ycombinator.com/item?id=49234271)

## How it works

A Python script runs on a schedule, pulls the highest-signal new dev content
(quality-filtered by stars, points, and reactions), files each item under a
topic headline, and commits it here. No duplicates — every entry is tracked.
